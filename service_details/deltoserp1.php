...
if (!$db_select) {
    die("Database selection failed: " . mysqli_error());
	 ...
	 $query="SELECT * from service_details where Item_Id='$a' and Delivered_To_Service_Provider_Date ='$da' and Company_To_Service_Provider_Date !='$da'";
	 $result1 = mysqli_query($dbh,$query) or die('Error: ' . mysqli_error($dbh));
	 echo"<table border size=1 >";
     echo"<tr><th>ITEM NAME</th><th> SERVICE REFERENCIAL ID</th><th>BARCODE NO</th><th>ACCOUNT_ID</th> <th>ITEM_ID</th><th>SENT DATE</th><th>COURIER NAME</th>";
      while ($brr=mysqli_fetch_row($result1))
	  {
		  echo"<tr> <td>$name1</td><td>$brr[0]</td> <td>$baa</td><td>$brr[2]</td> <td>$brr[3]</td><td>$brr[10]</td><td>$brr[27]</td></tr>";
	  } 
			}	  
?>
...
<p>
<p>
PRODUCT DELIVERED TO SERVICE PROVIDER:
<select name="status">
