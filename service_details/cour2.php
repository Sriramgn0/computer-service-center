...
if (!$db_select) {
	...
	?>
	<?php 
	 $query = "SELECT * from account_details where ACCOUNT_ID='$bad'";
     $result = mysqli_query($dbh,$query) or die('Error: ' . mysqli_error($dbh));
      while ($arr=mysqli_fetch_row($result))
	  {		  
		$baa=$arr[1];
...
<form method="post" action="cour3.php">
<font color="orange">
<p>
...
<p>
CUSTOMER ADDRESS<input type="text" name="cus1_Name" value="<?php $query = "select * from account_details where Account_Id='$bad'";
...
<p>
CUSTOMER ADDRESS<input type="text" name="cus_Name" value="<?php $query = "select * from account_details where Account_Id='$bad'";
																		$var=mysqli_query($dbh,$query);
																		while ($arr=mysqli_fetch_row($var))
																		{echo $arr[2];}
																		?>">
						<input type="text" name="cus_Name" value="<?php $query = "select * from account_details where Account_Id='$bad'";
																		$var=mysqli_query($dbh,$query);
																		while ($arr=mysqli_fetch_row($var))
																		{echo $arr[3];}
																		?>">
